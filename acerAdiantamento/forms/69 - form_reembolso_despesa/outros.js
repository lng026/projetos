	...
	if(adts.length){
		...
		for (var i = 0; i < adts.length; i++) {
			...
			if(a['saldo'] == a['valor']){
				...
				var AdtEmiss = dateFromString(a['emissao']);
...
var zoomAdiantamento = function(){
	...
	cpf = remove(cpf, ".");
	cpf = remove(cpf, "-");
	openZoom("Adiantamento", "dsAdiantamentos", "num,Codigo,emissao,Data,saldo,Saldo,valor,Valor","num,emissao,saldo,valor" , 'adiantamento',['cgc', cpf] );
...
function setSelectedZoomItem(selectedItem) {
	...
	switch (op) {
		...
		case 'despclientedesc':
			...
			break;	
		case 'adiantamento':
				$('#codAdiantamento').val(selectedItem.num);
			break;
			default:
