...
$(document).ready(function () {
	...
	$("#divAreasAnalise").find('input[type="checkbox"]').each((index, elem) => $(elem).attr("checked", false));

	// Limpa todos os campos de aprovação		
	$('select[name*="aprov"]').each((index, elem) => $(elem).val(""));
	...
	$('input[type="checkbox"][name="indeterminado"]').change(function () {
	...
	*/
	...
	if ($("#idStatus").val() != 34 || $("#idStatus").val() != 221) {
	...
	$("#dataTermino").on("change", function () {
		var valorDataI = $("#dataInicio").val().split("/")
		var valorDataF = $(this).val().split("/")
		if (valorDataI[0] > valorDataF[0]) {
			if (valorDataI[1] > valorDataF[1] || valorDataI[1] == valorDataF[1]) {
				if (valorDataI[2] > valorDataF[2] || valorDataI[2] == valorDataF[2]) {
	...
	if ($("#idStatus").val() > 5) {
		if ($("#dataTermino").val() == null || $("#dataTermino").val() == "") {
			$('#indeterminado').prop('checked', true);
		}
	}
	// Habilita Campos na atividade de analise juridica 

	if ($("#idStatus").val() == 5) {

		//campo Tipo Contrato 
		$("#tc")[0].children[3].className = "hidden form-control"
		$("#tipoDoc").show()
		selectTipoDoc()

		//campo minuta 
		$("#minuta")[0].children[2].className = "hidden form-control"
		$("#minutaPadrao").show()

		//Campo Data
		$("#dataInicio").attr('readonly', false)
		$("#dataTermino").attr('readonly', false)
		$('#indeterminado').attr('disabled', false)
		if ($("#dataTermino").val() == null || $("#dataTermino").val() == "") {
			$('#indeterminado').prop('checked', true);
			$("#dataTermino").attr("disabled", true);
		}
	...
	if ($("#idStatus").val() == 11) {
		...
		$('#btnRmvParte').show();
		$('#moedaValorMes').show();
		$("#rowMoeda")[0].children[1].children[2].className = 'ReadOnly hidden form-control'
		$('#valorMensal').attr('readonly', false)
	...
	var indexA = ($("#tbTrAnexos>tbody").children("tr").length) - 1;
	var indexC = ($("#tbTrContrato>tbody").children("tr").length) - 1;
	if ($("#idStatus").val() != 4 && $("#idStatus").val() != 0 && $("#idStatus").val() != 30 && $("#idStatus").val() != 106 && $("#idStatus").val() != 68) {

		$("#divAddAnexo").hide()
		$("#divAddContrato").hide()

		if (indexC == 0 || indexC < 0) {
			$("#pAnexoC").html("")
			$("#pAnexoC").html("Não foi anexado nenhum Contrato.")
		} else {
			$("#pAnexoC").html("")
		}
		...
		if (indexA == 0 || indexA < 0) {
			$("#pAnexo").html("")
			$("#pAnexo").html("Não foi anexado nenhum arquivo do Contrato Solicitado.")
		} else {
			$("#pAnexo").html("")
		}
	...
	// select
	if ($("#idStatus").val() == 4 || $("#idStatus").val() == 0) {
		$("#areaSol").append("<option value='' selected>Selecione a Área</option>");
	}

	if ($("#idStatus").val() == 61 || $("#idStatus").val() == 68 || $("#idStatus").val() == 96 || $("#idStatus").val() == 72 || $("#idStatus").val() == 108 || $("#idStatus").val() == 61 || $("#idStatus").val() == 204 || $("#idStatus").val() == 221 || $("#idStatus").val() == 106 || $("#idStatus").val() == 5|| $("#idStatus").val() == 30|| $("#idStatus").val() == 34|| $("#idStatus").val() == 143|| $("#idStatus").val() == 146|| $("#idStatus").val() == 151|| $("#idStatus").val() == 160|| $("#idStatus").val() == 188 || $("#idStatus").val() == 15|| $("#idStatus").val() == 51|| $("#idStatus").val() == 13|| $("#idStatus").val() == 48|| $("#idStatus").val() == 11|| $("#idStatus").val() == 56|| $("#idStatus").val() == 149 || $("#idStatus").val() == 150|| $("#idStatus").val() == 184|| $("#idStatus").val() == 187) {
		montaHistCompleto()
		$("#divAnaliseFin").show()
		if($("#idStatus").val() == 11){
			$("#divAnaliseFin legend").html("Análise Financeira")
		}else{
			$("#divAnaliseFin legend").html("Histórico de Mensagens")
		}
	...
	if ($("#hdnJsonRevisao").val()) {
		...
		$("#tbTrContrato>tbody").children("tr").each(function (index) {
			...
			if (isFinite(indexFromId(idRow))) {
				...
				var idDoc = $(this).find("input[id^='hdnIdContrato___']").val();
	...
	if ($("#hdnJsonRevisaoAnexo").val()) {
		...
		$("#tbTrAnexos>tbody").children("tr").each(function (index) {
			...
			if (isFinite(indexFromId(idRow))) {
				...
				var idDoc = $(this).find("input[id^='hdnIdAnexo___']").val();
...
function montaContratoVigente() {
	...
	if ($("#hdnJsonRevisao").val()) {
		...
		for (var cont = 0; cont < obj.length; cont++) {
			obj[cont].linkDownload = gerarLinkArquivo(obj[cont].idDoc);
			if (obj[cont].idAtividade == 34 || obj[cont].idAtividade == 221) {
				$("#tbContratoConsol>tbody").append(Mustache.render(template, obj[cont]));
				checkNull = false;
			}
		}
...
function setDivReadOnly(ids) {
	$.each(ids, function (key, value) {
		...
		$("#" + value).find("select").each(function (index, elem) {
			if ($(elem).parents('table').attr("tablename")) {
				var elemId = $(elem).attr('id').split('___')[0];
				var val = $(this).find("option[selected]").text();
				$(this).parent().append($("<input id=" + (elemId + '_input') + " name=" + (elemId + '_input') + " class='ReadOnly form-control' readonly='readonly' value='" + val + "'/>"));
				$(this).hide();
				return;
			}
			var val = $(this).find("option[selected]").text();
...
function selectTipoDoc() {
	...
	var optionSelected = $("#idTipoDoc").val()
	$("#tipoDoc").html("")
	for (var cont = 0; cont < dataset.values.length; cont++) {
		row = dataset.values[cont];
		if (optionSelected != null && optionSelected == row["tipoDoc"]) {
...
function statusAtv(atv) {
	...
	switch (atv) {
		...
		case '30':
			status = {
				'id': 30,  // Consolidação do contrato 
				'descricao': "Jurídico",
				'obs': "Jurídico",
				'abrv': "consolidado"
			};
		...
		case '34':
			status = {
				'id': 34, //Coleta de assinaturas
				'descricao': "Jurídico",
				'obs': "Jurídico",
				'abrv': "Assinatura Acer"
			};
			break;
		case '221':
			status = {
				'id': 221, //Coleta de assinaturas
				'descricao': "Jurídico",
				'obs': "Jurídico",
				'abrv': "Upload Contrato"
			};
			break;
		case '56':
			status = {
				'id': 56, //Revisão Financeiro
				'descricao': "Solicitante",
				'obs': "Solicitante",
				'abrv': "rev - fin"
			};
		...
		case '151':
			status = {
				'id': 151, //Revisão Jurídico - Financeiro
				'descricao': "Jurídico",
				'obs': "Jurídico",
				'abrv': "rev jur - fin"
			};
		...
		case '160':
			status = {
				'id': 160, //Revisão Jurídico - Fiscal
				'descricao': "Jurídico",
				'obs': "Jurídico",
				'abrv': "rev jur - fiscal"
			};
		...
		case '143':
			status = {
				'id': 143, //Revisão Jurídico - Logística
				'descricao': "Jurídico",
				'obs': "Jurídico",
				'abrv': "rev jur - log"
			};
		...
		case '96':
			status = {
				'id': 96, //Validação do Solicitante
				'descricao': "Solicitante",
		...
		case '48':
			status = {
				'id': 48, //Revisão Customer Service
				'descricao': "Solicitante",
		...
		case '146':
			status = {
				'id': 146, //Revisão Jurídico - Customer Service
				'descricao': "Jurídico",
		...
		case '188':
			status = {
				'id': 188, //Revisão Jurídico - Ecommerce
				'descricao': "Jurídico",
...
function adicionaPapeis() {
	...
	if (campoVazio) {
		...
		if (checkFirst) {
			var cnpjAcer = "11.068.167/0004-53";
			var descAcer = "AGP TECNOLOGIA EM INFORMÁTICA DO BRASIL LTDA";
			$("#cnpjPapel___" + id).val(cnpjAcer);
			$("#razaoSocial___" + id).val(descAcer);

			$('.col-md-12').remove(".cnpjAcer");

			// Seleção de CNPJ
			$("<div class='cnpjAcer' style='margin: 20px 0px;'>" +
				"<label class='control-label' for='cnpjAcer'>Selecione um CNPJ</label><br/>" +
				"	<div class='btn-group'>" +
				"		<label class='btn btn-large btn-info' for='cnpjAcer-0'>" +
				"		<input type='radio' name='cnpjAcer' id='cnpjAcer-0' value='11.068.167/0001-00'>11.068.167/0001-00</label>" +
				"		<label class='btn btn-large btn-info' for='cnpjAcer-1'>" +
				"		<input type='radio' name='cnpjAcer' id='cnpjAcer-1' value='11.068.167/0004-53'>11.068.167/0004-53</label>" +
				"		<label class='btn btn-large btn-info' for='cnpjAcer-2'>" +
				"		<input type='radio' name='cnpjAcer' id='cnpjAcer-2' value='11.068.167/0005-34'>11.068.167/0005-34</label>" +
				"		<label class='btn btn-large btn-info' for='cnpjAcer-3'>" +
				"		<input type='radio' name='cnpjAcer' id='cnpjAcer-3' value='11.068.167/0006-15'>11.068.167/0006-15</label>" +
				"		</div>" +
				"	</div>").insertBefore("#tbOutrasPartes");

			$(".cnpjAcer").on('change', 'input[name="cnpjAcer"]:checked', function () {
				$("#cnpjPapel___" + id).val($("input[name='cnpjAcer']:checked").val());
				$('.cnpjAcer').remove(".cnpjAcer");
			});
		}
...
var beforeSendValidate = function (numState) {
	...
	if (numState == 204) {
		validaAprovacao('aprovPresidenciaObs', 'Observações');
		histAnaliseAreas("Presidencia", "analise");
	}
	if (numState == 5) {
	...
	if (numState == 96) {
		histAnaliseAreas("Juridico", "solicitante");
	}
	if (numState == 30) {
		//validaAnexosConsol();
	}
...
function histAnaliseAreas(area, tarefa) {
	...
	else if (tarefa == "juridico" && area == "Juridico") {
		msg = $("#obs" + area + "").val();
	}
	else if (tarefa == "solicitante" && area == "Juridico") {	
		msg = $("#aprovSolObs").val();	
	}
	else {
...
function montaHistCompleto() {
	...
	var presidencia = $("#hdnJsonHistPresidencia").val() ? JSON.parse($("#hdnJsonHistPresidencia").val()) : "";
	...
	if (presidencia == "") { presidencia = JSON.parse('{"sequencia":"Não utlizado","msg":"Não utlizado","autor":"Não utlizado","data":"Não utlizado","atividade":" Aprovação Presidência","area":"Não utlizado"}'); }
	if (presidencia != "") { presidencia = $("#hdnJsonHistPresidencia").val() ? JSON.parse($("#hdnJsonHistPresidencia").val()) : ""; }

	var historico = superior.concat(juridico, fin, cs, log, eCommerce, fiscal, presidencia);
	...
	$('.btnAddAnexo').fileupload({
		dataType: 'json',
			...
			if ($("#tipoDoc").val()) {
				...
				if ($("#idStatus").val() == 4 || $("#idStatus").val() == 0 || $("#idStatus").val() == 30 || $("#idStatus").val() == 106) {
					$(".btnAddRevisaoAnexo").closest("a").hide();
...
const openDocument = (item) => {
	var indice = item.id.split("___")[1];
	if (indice == undefined) {
		if ($(item).is('span')) indice = $(item).parent().parent().parent().find('span')[1].id.split('___')[1];
		else indice = $(item).parent().parent().parent().find('input').first().attr('id').split('___')[1]
	}
	var verificarBtn = item.id.split("___")[0];
	var docId = ""

	if (verificarBtn == 'btnAddAnexo') {
		docId = $("#hdnIdAnexo___" + indice).val() == '' ? $("#hdnIdAnexo___" + indice).text() : $("#hdnIdAnexo___" + indice).val();
	} else {
		docId = $("#hdnIdContrato___" + indice).val() == '' ? $("#hdnIdContrato___" + indice).text() : $("#hdnIdContrato___" + indice).val();
	}
...
const openDocumentRevisao = (item) => {
	var indice = item.id.split("___")[1].split("_");
	var pai = item.id
	var idDoc = $("#" + pai).parent().parent().parent()[0].children[1].children[1].value
	...
	var cfg = {
		...
		title: "Visualizador de Documentos",
		callBack: function () {
			parentOBJ.ECM.documentView.getDocument(idDoc, 1000);
		},
		customButtons: []
	};
	parentOBJ.ECM.documentView.panel = parentOBJ.WCMC.panel(cfg);
...
const openDocumentConsolidado = (item) => {
...
const openDocumentEncerrado = (item) => {

	var idDoc = item.value
...
function uploadDocEncerrado() {
	$('[name="btnAddDocEncerrado"]').fileupload({
		dataType: 'json',
		done: function (e, data) {
			var myLoading2 = FLUIGC.loading(window);
			myLoading2.show();
			...
			$.each(data.result.files, function (index, file) {
				...
				$.ajax({
					...
					success: function (data) {
						...
						var urlEncerrado = gerarLinkArquivo(data.content.id)
						$("#hdnUrlDocEncerrado___" + id).val(urlEncerrado);
						$("#btnDownDocEncerrado___" + id).attr("href", urlEncerrado);
						$("#inputVisualizarDocEncerrado___" + id).val(data.content.id);
...
function trDeleteDocContratoExp(elem) {

	var index = elem.id.split('___')[1]
	...
	FLUIGC.message.confirm({
		...
		if (result) {
			var idDoc = $("#inputVisualizarDocEncerrado___" + index).val();
...
function validaAnexosVigente() {
	...
	if (docsContrato) {
		for (var cont = 0; cont < docsContrato.length; cont++) {
			...
			checkContrato = docsContrato[cont].idAtividade == 221 ? true : false;
		}
	}
	...
	if (docsAnexo) {
		...
		for (var cont = 0; cont < docsAnexo.length; cont++) {
			...
			checkAnexo = docsAnexo[cont].idAtividade == 221 ? true : false;
		}
	}
