...
//    customHTML.append('defineAprovador();');
	...
	if(form.getFormMode() != "VIEW")  {
		...
		form.setVisibleById("aprovGestorDiv", (atividade == 5 || atividade == 45 || atividade == 46 || atividade  == 67|| atividade  == 98 || atividade  == 83 ));
		if(atividade == 5 || atividade == 45 || atividade  == 98 ){
			...
			customHTML.append("$('#aprovGestorDiv').find('select').each(function(){ $(this).attr('disabled', false);});");
			customHTML.append("</script>");
		}else{
			customHTML.append("<script>");
			customHTML.append("$('#aprovGestorDiv').find('input, textarea').each(function(){ $(this).attr('readonly', true);});");
			customHTML.append("$('#aprovGestorDiv').find(':radio').each(function(){ $(this).attr('disabled', true);});");
			customHTML.append("$('#aprovGestorDiv').find('select').each(function(){ $(this).attr('disabled', true);});");
			customHTML.append("</script>");
		}
		...
		if(atividade == 46 ){
			...
			customHTML.append("$('#aprovGermanoDiv').find('select').each(function(){ $(this).attr('disabled', false);});");
			customHTML.append("</script>");
		}else{
			customHTML.append("<script>");
			customHTML.append("$('#aprovGermanoDiv').find('input, textarea').each(function(){ $(this).attr('readonly', true);});");
			customHTML.append("$('#aprovGermanoDiv').find(':radio').each(function(){ $(this).attr('disabled', true);});");
			customHTML.append("$('#aprovGermanoDiv').find('select').each(function(){ $(this).attr('disabled', true);});");
			customHTML.append("</script>");
		}
		
		//exibe e habilida edicao aprovacao RH == 90) );
		form.setVisibleById("aprovRHDiv", atividade == 90 || atividade  == 98 || atividade  == 83  );
		if(atividade == 90){
			customHTML.append("<script>");
