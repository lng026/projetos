var camposObrigatorios = ['nome','cpf','departamento','banco','agencia','conta','centro_custo','finalidade','valor'];
var nomeCampos = [];
...
nomeCampos['finalidade'] = "Finalidade do adiantamento";
nomeCampos['valor'] = " Valor R$";
...
$(document).ready(function(){
	...
	$('#cpf').on('focusout',function(){
		...
		if(!ok){
			...
			buscaDados(cpf);
			var adtAnt = verAdtAnteriores();
			if(adtAnt){
			FLUIGC.toast({title: 'Adiantamentos Anteriores:', 
			message: 'Existem adiantamentos e aberto para o usuário.', type: 'warning'});
	...
	$('.linha').show();
	$('.linha').first().hide();
	// Exibe feedback de aprovacoes
	if($("#aprovGestorObs").val().trim() != ""){
		$("#aprovGestorDiv").show();
		$("#aprovdiv").show();
	}
	if($("#aprovGermanoObs").val().trim() != ""){
		$("#aprovGermanoDiv").show();
		$("#aprovdiv").show();
	}
	...
	FLUIGC.calendar('#dtPartida');
	FLUIGC.calendar('#dtRetorno');
});


// Altera o campo KM na view com base na conta contábil
$(document).ready(function(){
	if (FORM_MODE == 'VIEW') {
		var objeto = $(".despctacontabil");
		$.each(objeto, function( key, value ) {
			var texto = value.innerHTML;
			if(texto == "313020024") {
				var textoId = $(this).attr('id'); 
				var idSubstuir = textoId.replace("despctacontabil___", "");
				recebeTexto = objeto.parent().next("td").next("td").next("td").children("span#despvalorporkm___" + idSubstuir).text();
				textoAlterado = recebeTexto.replace(".",""); 
				objeto.parent().next("td").next("td").next("td").children("span#despvalorporkm___" + idSubstuir).text(textoAlterado + " KM");
			}
		});
	}


	// Verificar e definir aprovador
...
function calcTotalGeral(){
	...
	somaComMenosDolar('.valor','#total2','#dolaroureal');
}
...
var somaComMenosDolar = function(campo,id,campodolar){
	...
	//$(id).empty();
	$(id).val(MaskMonetario(diferenca.toFixed(2)));
	aplicaMasks();
	
}

function MaskMonetario(diferenca){
	v = diferenca.toString();
	v=v.replace(/\D/g,""); //Remove tudo o que não é dígito
	v=v.replace(/(\d{2})$/,".$1"); //Coloca a virgula
	v=v.replace(/(\d+)(\d{3},\d{2})$/g,"$1,$2"); //Coloca o primeiro ponto
	var qtdLoop = (v.length-3)/3;
	var count = 0;
	var resul = diferenca < 0 ? "-" : "";
	while (qtdLoop > count){
		count++;
...
var beforeSendValidate = function(numState) {
	...
	if(numState <= 4){
		...
		var adtParams = getCustomParams("adtDesp");
		$("#natureza").val(adtParams['natureza']);
		$("#prefixo").val(adtParams['prefixo']);
		$("#formPag").val(adtParams['formPag']);
		$("#valorMax").val(adtParams['valorMax']);
		$("#cnpjPag").val(adtParams['cnpjPag']);
	}
	validaForm();//valida cabeçalho
	// Verifica valor maximo
	var formVal = formataValor($("#valor").val());
	var valorMax = formataValor($("#valorMax").val());
	if(parseFloat(formVal) > parseFloat(valorMax)){
		throw "O Valor solicitado ultrapassa o valor máximo permitido de: R$"+valorMax + ".";
	}
	...
	$("#nValor").val(formataValor($("#valor").val()));
}

// 000.000,00 -> 000000.00
function formataValor(valor){
	var aux = semCaractere(valor,'.');
	aux = aux.replace(',','.');
	return aux;
}
...
var defineAprovador = function(){
	...
	var aprovador = getAprovSolicitante(uAtualID);
	//console.clear();
	//console.log(aprovador);
	
	//se aprovador nao for encontrado tarefa é enviada para usuario admin
	if(aprovador != ''){
		var status = getUserStatus(aprovador);
		if(status){
			$("#aprovador").val(aprovador);
		}
	}else{
		alert('Usuário não possui aprovador. Por favor entre em contato com a equipe de TI.');
	}
}

//verifica se existem adiantamnetos abertos do usuários
var verAdtAnteriores = function(){
	var hasAdtAberto = false;
	var cpf = $("#cpf").val();
	cpf = remove(cpf, ".");
	cpf = remove(cpf, "-");
	var adts = buscaAdiantamentos(cpf);
	if(adts.length){
		// verifica se existem adiantamentos com saldo igual ao valor
...
var validaAprovacao = function(){
	// verificar obrigatoriedade dos campos de obsevação da aprovação
}
...
var setRHExiste = function(){
	...
	list.forEach(function(e,i){
		// console.log('index:'+i); 314020027
		console.clear();
		console.log("Lista: " + e.value);
		if(e.value){
...
function getAprovSolicitante(sol){
	var aprov = '';
	var cs1 = DatasetFactory.createConstraint('solicitante', sol, sol, ConstraintType.MUST);
	var cs2 = DatasetFactory.createConstraint('status', '1', '1', ConstraintType.MUST);
	var dsAprovador = DatasetFactory.getDataset('dsCadastroAprovadores', null, new Array(cs1,cs2), null);
	if(dsAprovador.values.length){
		aprov = dsAprovador.values[0]['aprovador'];
	}
	return aprov;
 }
 ...
 function getUserStatus(id){
	var status = 0;
	var cs1 = DatasetFactory.createConstraint('active', 'true', 'true', ConstraintType.MUST);
	var cs2 = DatasetFactory.createConstraint('colleaguePK.colleagueId', id, id, ConstraintType.MUST);
	var dsUser = DatasetFactory.getDataset('colleague', null, new Array(cs1, cs2), null);
	if(dsUser.values.length){
		status = 1;
	}
	return status;
 }
