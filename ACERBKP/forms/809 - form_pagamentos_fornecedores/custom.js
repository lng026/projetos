...
$(document).ready(function(){
	...
	if(view != "VIEW"){
		...
		case finalizar:
			...
			setDivReadOnly(["rateiosCentrodeCusto", "solicitante", "manager", "supply","listaNFEs"]);
		...
		exibeUltimosAprovs();
			...
			setDivReadOnly(["rateiosCentrodeCusto", "financeiro", "solicitante", "supply", "manager","listaNFEs"]);
			break;	
		case diretoria:
				$('#diretoria').show();
				$('#financeiro').hide();
	...
	$('#btZoomFornecedor').click(function(){
		...
		// }else{
			...
			if(cForn.length >= 3){
				...
				FLUIGC.toast({title: 'Aviso', message: 'Informe ao menos 3 digitos do CPF/CNPJ do Fornecedor desejado.', type: 'warning'});
			}
	...
	$('#BtZoomNfe').click(function(){
		// var cCusto   = $('#cCentroCusto').val();
		var cgcForn = getFornSel();
		openZoom("Nota Fiscal Eletronica", "dsChaveNFE", 
		"CNOTAFISCAL,Nota,NVALOR,Valor,CSERIENOTA,Serie,CCHAVENFE,Chave",
		"CCGC,CCHAVENFE,CDATAEMISSAO,CSERIENOTA,CNOTAFISCAL,NVALOR", "cteFornecedor", ["CCGC",cgcForn]);
});
...
function setSelectedZoomItem(selectedItem) {
	...
	if(selectedItem.type == "cteFornecedor"){
		addLinhaNfeZoom(selectedItem);
	}
...
function exibeUltimosAprovs(){
	...
	var forn = document.querySelector("#cCodForn");
...
function getPagForn(forn,cco){
	var c1 = DatasetFactory.createConstraint('cCodForn', forn, forn, ConstraintType.MUST);
...
function newTRpa(pa){
    ...
    nTr.appendChild(newTd(pa['NrSolic'] != null ? newSolLink(pa['NrSolic'])  : '-'));
...
function newSolLink(n){
	var link = '<a href="/portal/p/1/pageworkflowview?app_ecm_workflowview_detailsProcessInstanceID='+n+'" target="_blank">'+n+'</a>';
	return link;
}
