...
var multiplicaCampo = function(valor,multiplicador,resultado,classe){
	...
	console.log('campo_total ' + campo_total);

	var multiplicadorKM = verificaKM(valor);
	console.log(multiplicadorKM);
...
var somaPorDespesa = function(campo1,campo2){

	var tamanho = $("#tabela tbody tr").length;
	var array1 = $(campo1).toArray();
	var array2 = $(campo2).toArray();
	...
	for (var i = 1; i < array1.length; i++) {
		var valor = parseFloat($(array1[i]).val().replace(',','').replace(',',''));
		console.log(valor);
		var contabil = parseInt($(array2[i]).val().replace(',','').replace(',',''));
		console.log(contabil);

		//somatoria = somatoria+valor;
		//console.log(somatoria);

		if (contabil == "314020023"){
			console.log('despesas de viagens');

			viagem = viagem + valor;
			somacat = somacat + viagem;

			$("#viagem").empty();
			$("#viagem").val(viagem.toFixed(2));
		}
		else if (contabil == 314020024) {
			console.log('reembolso de km');

			quilometro = quilometro + valor;
			somacat = somacat + quilometro;

			$("#quilometro").empty();
			$("#quilometro").val(quilometro.toFixed(2));
		}
		else if (contabil == 314020025) {
			console.log('lanches e refeições');

			refeicao = refeicao + valor;
			somacat = somacat + refeicao;

			$("#refeicao").empty();
			$("#refeicao").val(refeicao.toFixed(2));
		}
		else if (contabil == 314020026) {
			console.log('estacionamento e pedagio');

			pedagio = pedagio + valor;
			somacat = somacat + pedagio;

			$("#pedagio").empty();
			$("#pedagio").val(pedagio.toFixed(2));
		}
		else if (contabil == 314020028) {
			console.log('bens de pequenos valor');

			pequeno_valor = pequeno_valor + valor;
			somacat = somacat + pequeno_valor;

			$("#pequeno_valor").empty();
			$("#pequeno_valor").val(pequeno_valor.toFixed(2));
		}
		else if (contabil == 314020031) {
			console.log('material de escritorio');

			escritorio = escritorio + valor;
			somacat = somacat + escritorio;

			$("#escritorio").empty();
			$("#escritorio").val(escritorio.toFixed(2));
		}
		else if (contabil == 314020034) {
			console.log('correio');

			correio = correio + valor;
			somacat = somacat + correio;

			$("#correio").empty();
			$("#correio").val(correio.toFixed(2));
		}
		else if (contabil == 314020021) {
			console.log('transporte de passageiros');

			transporte = transporte + soma;
			somacat = somacat + transporte;

			$("#transporte").empty();
			$("#transporte").val(transporte.toFixed(2));
		}
		else if (contabil == 314020032) {
			console.log('material de informatica');

			informatica = informatica + valor;
			somacat = somacat + informatica;

			$("#informatica").empty();
			$("#informatica").val(informatica.toFixed(2));
		}
		else {
	...
	$("#totalcat").empty();
	$("#totalcat").val(somacat.toFixed(2));
	aplicaMasks();
