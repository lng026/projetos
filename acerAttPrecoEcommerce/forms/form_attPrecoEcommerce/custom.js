...
function addTrAnyM(mp){
	var trbody = `<tr class="trPriceAnyM" id="${mp.id}"> 
		...
		<td><input type="text" class="form-control" id="anyMPrice" value="${trataValor(mp.price)}" readonly/></td>
...
function savePrecoAnyM(){
	...
	trs.each((id,tr) => {
		...
		anyObj = newAnymarketPrice(id,anyMPrice,anyMDiscountPrice,anyMPriceFactor,anyMEditchk,anyMStatus,anyMSaleChannel);
		anyPrecos.push(anyObj);
	});
	$("#precoAnyMNovo").val(JSON.stringify(anyPrecos));
}

function newAnymarketPrice(_id,_price,_discount,_priceFactor,_edit,_status,_saleChannel){
	obj = {
		'id' : _id,
		'price' :_price,
...
function tlgEditAnyM(e){
	...
	$(tr).find('#anyMPriceFactor').attr('readonly',!e.checked);
}
