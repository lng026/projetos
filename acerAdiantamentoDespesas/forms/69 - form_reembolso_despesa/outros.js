var camposObrigatorios = ['nome','cpf','departamento','banco','agencia','conta','centro_custo','finalidade'];
var nomeCampos = [];
...
var buscaDados = function(param){
	...
	if(fun['CAGENCIA'].toUpperCase() !=  'ERRO' && fun['CAGENCIA'].trim() != ''){
		...
		// Verificar adiantamentos
			var adtAnt = verAdtAnteriores(0);
			if(adtAnt){
				var message = 'Existem adiantamentos em aberto em nome do usuário selecionado.';
				FLUIGC.toast({title: '', message: message, type: 'warning'});
			}
		}else{
...
var beforeSendValidate = function(numState) {
	...
	// se não foi selecionado adiantamneto
	if(!$("#codAdiantamento").val().trim()){
		var adtAnt = verAdtAnteriores();
		if(adtAnt){
			var message = 'Existem adiantamentos em aberto com mais de 30 dias em nome do usuário selecionado.';
			message += '\nPor gentileza, selecione um adiantamnto.';
			throw message;
		}
	}
...
var verAdtAnteriores = function(dias = 30){
	...
	if(adts.length){
		...
		for (var i = 0; i < adts.length; i++) {
			var a = adts[i];
			// verifica se adiantamento está aberto
			//if(a['saldo'] == a['valor']){
				// verificar se tem 30 mais de 30 dias corridos
				var dtLimite = getDataPassada(dias);
				var AdtEmiss = new Date(a['emissao']);
				if(AdtEmiss <= dtLimite){
					hasAdtAberto = true;
				}
			//}
		}
...
var zoomAdiantamento = function(){
	var cpf = $("#cpf").val();
	cpf = remove(cpf, ".");
	cpf = remove(cpf, "-");
	openZoom("Adiantamento", "dsAdiantamentos", "num,Codigo,emissao,Data,saldo,Saldo,valor,Valor,xfluig,ID-Fluig","num,emissao,saldo,valor,xfluig" , 'adiantamento',['cgc', cpf] );
...
function setSelectedZoomItem(selectedItem) {
	...
	switch (op) {
		...
		case 'despclientedesc':
			...
			break;	
		case 'adiantamento':
				$('#codAdiantamento').val(selectedItem.xfluig);
			break;
			default:
