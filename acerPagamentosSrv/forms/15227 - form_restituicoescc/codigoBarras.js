function f_barra() {
    ...
    // var antes  = nBolCodBar.value;
    var depois = calcula_barra(nBoleto.value);
   if(depois){   
       $("#nBolCodBar").val(depois);
...
function f_linha() {
    var antes  = nBoleto.value.replace(/[^0-9]/g,'');
    var depois = calcula_linha(nBolCodBar.value);
    nBoleto.value=depois;
...
function f_venc() {
    if ( nBolCodBar.value.substr(5,4) == 0 )
        VencData.value='O boleto pode ser pago em qualquer data';
    else
        VencData.value=fator_vencimento(nBolCodBar.value.substr(5,4));
        ValTitulo.value=(nBolCodBar.value.substr(9,8)*1)+','+nBolCodBar.value.substr(17,2);
    return(false);
}
function calcula_barra(linha)
{
    ...
    if (modulo11_banco(barra.substr(0,4)+barra.substr(5,39)) != barra.substr(4,1)){
        ...
        return false;
        //if (nBolCodBar.value != nBolCodBar2.value) alerta('Barras diferentes');
    }
        return(barra);
}
function calcula_linha(barra)
{
    //var barra = nBolCodBar.value;	// Codigo da Barra
