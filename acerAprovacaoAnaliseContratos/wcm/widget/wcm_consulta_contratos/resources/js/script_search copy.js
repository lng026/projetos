...
function searchValues(callback, detail) {
	...
	try {
		...
		// Solicitação
		if (buscNumSoli) {
			oC1.push(getHtmlCons('numSolicitacao',buscNumSoli,buscNumSoli,0,false));
		}
		// Status 
		if (buscStatusSoli) {
			// ignora encerrado e cancelado
			if (buscStatusSoli != '66' && buscStatusSoli != '36') {
				oC1.push(getHtmlCons('idStatus',buscStatusSoli,buscStatusSoli,0,false));
			}
		}

		// Área solicitante
		if (areaSolicitante) {
			oC1.push(getHtmlCons('areaSol',areaSolicitante,areaSolicitante,0,false));
		}
		// Tipo de contrato
		if (buscaTipoContrato) {
			oC1.push(getHtmlCons('idTipoDoc',"%" + buscaTipoContrato + "%","%" + buscaTipoContrato + "%",0,true));
			oC1.push(getHtmlCons('tipoDoc',"%" + buscaTipoContrato + "%","%" + buscaTipoContrato + "%",0,true));

		}
		// Solicitante
		if (buscaSolicitante) {
			//console.log("Entrou 'buscaSolicitante' " + buscaSolicitante);
			oC1.push(getHtmlCons('usrSolicitante',buscaSolicitante,buscaSolicitante,0,false));
		}
		...
		if (buscaRazaoSocial) {
			oC1.push(getHtmlCons('relacaoDePartes',buscaRazaoSocial,buscaRazaoSocial,0,true));
		}
		 ...
		 if (!buscNumSoli && !buscStatusSoli && !buscaTipoContrato && !buscaSolicitante && !buscaRazaoSocial && !dataInicial && !dataFinal && !areaSolicitante) {
			...
			oC1.push(getHtmlCons('metadata#active',true,true,0,false));
			...
			table = $('#gridSolicitacoes').DataTable({
				...
				"columns": [
					...
					// Status da atividade
					{
						...
						"orderable": false,
						"render": function (data, type, full, meta) {
							var atvParalelas = [15,51,143,13,48,146,11,56,151,159,150,160,184,187,188];
							if(atvParalelas.includes(parseInt(full.idStatus))){
								return "<p>" + getAtvAtualProcess(full.numSolicitacao) + "</p>";
							}else{
