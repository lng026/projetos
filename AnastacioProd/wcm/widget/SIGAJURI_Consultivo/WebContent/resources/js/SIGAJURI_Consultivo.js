var wcmConsultivo = SuperWidget.extend({
    ...
    mostraZoom: function(el,ev){
    	...
    	case "zoomUsuarioResponsavel_"+this.instanceId:
    		...
    		zoomFieldHeader = "${i18n.getTranslation('application.zoom.Usuario.Header')}";
    		break;
    	}
    	
    	// Abre a janela de zoom de acordo com as variaveis determinadas.
    	window.open("/webdesk/zoom.jsp?datasetId=" + ds + "&dataFields=colleagueName, " + zoomFieldHeader + "&resultFields=mail,colleagueName&type=" + type + "&title=" + zoomTitle, "zoom", "status , scrollbars=no ,width=600, height=350 , top=0 , left=0");
    ...
    updateDatatableCons: function(){
    	...
    	var fields = new Array('metadata#id', 'cdAssJur', 'sAssJur', 'cdTipoSol', 'sTipoSol', 'cdUser', 'sUser', 'sPrioridade', 'sPrazo');
    	var constraints = new Array();
    	var sort = new Array();
    ...
    validateFields: function(){
	    ...
	    if (cdUser == ''){ // Valida se o campo Usuario foi preenchido.
	    	msg += "<li>${i18n.getTranslation('application.label.UsuarioResp')}</li>";
	    	success = false;
	    }
