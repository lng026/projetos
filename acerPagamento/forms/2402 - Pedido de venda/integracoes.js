var openZoomGerente = function(el){
  ...
  function zoomTransp(element){
    var div = element.parentElement.parentElement;
    var inputTransp = div.querySelector(".pv_transp");
    var id = inputTransp.attributes.id.value;
    var index = indexFromId(id);
    var fil = inputTransp.value;
    openZoom("Produto", "dsTransportadora", "CCODIGO,CODIGO,CNOME,NOME","CCODIGO,CNOME" , ['transportadora', index] , ["CCODIGO", fil ]);
...
function setSelectedZoomItem(selectedItem) {
  ...
  var tip = selectedItem.type != undefined ? selectedItem.type : selectedItem.inputName;
    ...
    case 'transportadora':
    var i = arrayTipo[1];
    $('#pv_transCod___'+i).val(selectedItem.CCODIGO);
    $('#pv_transp___'+i).val(selectedItem.CNOME);
    break;
		default:
