function servicetask17(attempt, message) {
	...
	params.push(getCons('CNPJ','11068167000615'));
	...
	params.push(getCons('cAgencia',form.get('agencia')));
		...
		if(DsInsPagCli.rowsCount){
			...
			if(result && result.trim()){
				if(result.trim().indexOf('SRV') >= 0){
					...
					log.info('Titulo não inserido: ' + oRes.errorMessage);
					throw "Titulo não inserido: "+ oRes.errorMessage;
				}
			}
		}
