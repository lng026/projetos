...
function createDataset(fields, constraints, sortFields) {
	...
	/*
	 * dataset.addColumn("Cond_normal"); dataset.addColumn("Centro_de_custo");
	 */
	...
	if ((param != '' && param.length() > 2)
		...
		try {
			...
			var cliResponse = service.getclientes(param.toUpperCase(), param2
					.trim(), "");
			var cli = cliResponse.getCLIENTES();
			// dataset.addRow(new
			// Array(brw.get(0).toString(),brw.get(0).getDESCRIPTION()));
			for (var i = 0; i < cli.size(); i++) {
				dataset.addRow(new Array(cli.get(i).getCCODIGO().trim(), cli
						.get(i).getCNOME().trim(), cli.get(i).getCCGC().trim(),
						cli.get(i).getCMUNICIPIO().trim(), cli.get(i)
								.getCENDERECO().trim(), ''));
			}
