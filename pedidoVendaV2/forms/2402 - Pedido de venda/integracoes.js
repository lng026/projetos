var openZoomGerente = function(el) {
	...
	openZoom("Cliente", "dsUsersRole", "colleagueId,ID,colleagueName,Nome",
			"colleagueId,colleagueName", [ 'gerente', i ],
			[ 'roleId', 'papel1' ]);
}
...
var zoomCliente = function(el) {
	...
	// openZoom(title, dataset, fields, resultFields, type, filters)
	openZoom("Cliente", "dsCliente", "CCODIGO,Codigo,CNOME,Nome,CCGC,CGC",
			"CCODIGO,CNOME,CCGC,CMUNICIPIO,CENDERECO", 'cliente', [ 'CCGC',
					cnpjCli ]);
}
...
var zoomProduto = function(el, field, filter) {
	...
	if (fil.trim() == '' || fil.length < 3) {
		FLUIGC.toast({
					title : 'Busca:',
					message : 'Por favor, Digite ao menos as 3 primeiras letras da descrição do produto.',
					type : 'warning'
				});
		return 0;
	}
	...
	openZoom(
			"Produto",
			"dsProdutos",
			"DESCRIPTION,DESCRICAO,PRODUCTCODE,CODIGO,DESCRIPTIONGROUPOFPRODUCT,GRUPO",
			"PRODUCTCODE,DESCRIPTION,TYPEOFPRODUCT,DESCRIPTIONGROUPOFPRODUCT,QUANTITYPERPACKAGE",
			[ 'produto', i ], [ filter, fil ]);
}
...
var zoomCli = function(el) {
	...
	if (fil.trim() == '' || fil.length < 3) {
		FLUIGC
				.toast({
					title : 'Busca:',
					message : 'Por favor, Digite ao menos as 3 primeiras letrar no nome fantasia do cliente.',
					type : 'warning'
				});
		return 0;
	}
	...
	// openZoom(title, dataset, fields, resultFields, type, filters)
	openZoom("Cliente", "dsCliente", "CCODIGO,Codigo,CNOME,Nome,CCGC,CGC",
			"CCODIGO,CNOME,CCGC,CMUNICIPIO,CENDERECO", 'cliente',
			[ 'nome', fil ]);
}
...
/**
 * @Overload de openZoom Abre janela de zoom. +
 * @param windowParams:
 *            Parâmetros da janela de zoom.
 */
function openZoomWindowParam(title, dataset, fields, resultFields, type,
		filters) {
	var windowParams = "status , scrollbars=no ,width=800, height=350 , top=0 , left=0";
	var zoomURL = "/webdesk/zoom.jsp?datasetId=" + dataset + "&dataFields="
			+ escape(fields) + "&resultFields=" + resultFields + "&type="
			+ type + "&title=" + title
...
function setSelectedZoomItem(selectedItem) {
	...
	var tip = selectedItem.type != undefined ? selectedItem.type
